        DEFINE  autocarga	;For Autoload ROM
		DEFINE  nocheckram	;Does not execute ram checking
OVERALL_COLOURS	EQU	%00111111		;White paper, black ink for loading
LOADBORDER		EQU	%00001001		; Border White, does not touch bits for EAR/MIC, not required
...
;   often used by third-party software. 
		...
		ORG 0000
	;;IFDEF autocarga
	;;	output "48k_AUTOLOAD.rom"
	;;ELSE
		output "48k.rom"
	;;ENDIF

	IFDEF nocheckram
HLclrPAG	EQU		#FE	;Apparently is enough clearing from #4000-#CDCD, anyway will clear upto #FEFE
HLfrom		EQU		(HLclrPAG<<8)+(HLclrPAG)+1
BCfrom		EQU		HLfrom -#4000
	ELSE
HLfrom		EQU		$FFFF
...
L0000:  DI                      ; Disable Interrupts.
      ...
      IFDEF autocarga
		LD      HL,HLfrom       ; Set pointer to top of possible physical RAM.
...
L0556:  INC     D               ; reset the zero flag without disturbing carry.
        ...
        LD      A,LOADBORDER	; $0F           ; make the border white and mic off.
...
L11C8:
			...
			IFDEF nocheckram		;nocheckram
		;IM 		1	
		LD		(HL),A			;A=0 
        OUT     ($FE),A         ; and set it now by writing to a port. (A=0 so border 0)
		
		EX      AF,AF			; Save the flag to control later branching. '
        ;LD      A,$38           ; Select a black border
        ;OUT     ($FE),A         ; and set it now by writing to a port.

        ;LD      A,$3F           ; I interrupt (only for IM2)
        ;LD      I,A             ; Set the I register - this remains constant
                                ; and can't be in the range $40 - $7F as 'snow'
                                ; appears on the screen.
		;LD		HL,#FFFF		;clear Screen zone (and a bit more to adjust clear zone: HL=#5B5C, DE=#5B5B, BC=#1B5B)
		;LD 	DE,#5B5B
		LD		D,H
		LD		E,H
		;LD 		BC,#1AFF
		...
		LD		H,E
		LD		L,E
		LD		DE,#1F05		;Necessary, if DE is not #1F05 then some corrupted text appears when loading
		DEFS $11DB-$,0
...
;   intact.
      ...
      ;  LD      (IY-3),$3C      ; character set, CHARS - as no printing yet.
        LD      HL,$3C00        ; a strange place to set the pointer to the ;Mad3001 Replaced previos IY-3 to be sure low and ...
        LD      ($5C36),HL      ; character set, CHARS - as no printing yet.;Mad3001 ... high byte of Charset initialization
        ...
        LD      ($5C65),HL      ; set STKEND to the end of the empty stack.
                                ; --
        LD      A,OVERALL_COLOURS	;$38           ; the colour system is set to white paper,
...
;        LD      ($5C8F),A       ; set ATTR_T temporary colour attributes.
		...
		DEFM    'ResetPlay,Antonio Villena 2012'; 30 bytes
...
L1CA5:  SUB     LOW (L1AEB-$D8) ; convert $EB to $D8 ('INK') etc.
